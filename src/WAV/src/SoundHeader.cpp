...
FileIO& operator<<(FileIO& output, SoundHeader& header) {
   switch (header.getOutputType()) {
      ...
      case TYPE_SND:
         ...
         break;
      case TYPE_WAV_PCM:
         output.writeBigEndian((uint32_t)0x52494646);      // "RIFF"
         output.writeLittleEndian((uint32_t)(36 + header.getBitsPerSample()
            / 8.0  * header.getChannels() * header.getSamples()));
         output.writeBigEndian((uint32_t)0x57415645);      // "WAVE"
         output.writeBigEndian((uint32_t)0x666d7420);      // "fmt "
         output.writeLittleEndian((uint32_t)16);           // fmt subchunk size
         output.writeLittleEndian((uint16_t)1);           // PCM format
         output.writeLittleEndian((uint16_t)header.getChannels()); // channels
         output.writeLittleEndian((uint32_t)header.getSrate());  // sampling rate
         output.writeLittleEndian(
            (uint32_t)(header.getSrate()* header.getChannels() *
            header.getBitsPerSample() / 8));  // byte rate
         output.writeLittleEndian((uint16_t) (header.getBlockAlign()));
         output.writeLittleEndian((uint16_t)header.getBitsPerSample());
         output.writeBigEndian((uint32_t)0x64617461);   // "data"
         output.writeLittleEndian((uint32_t)(header.getSamples() *

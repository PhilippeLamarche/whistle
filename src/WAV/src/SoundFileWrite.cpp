...
//////////////////////////////
//
// SoundFileWrite::Construct --
//

void SoundFileWrite::Construct(const char* aFileName, SoundHeader& header)
{
...
void SoundFileWrite::Construct(const char* aFileName) {
    SoundHeader header;
    header.setHighMono();
    Construct(aFileName, header);
}
...
SoundFileWrite::SoundFileWrite(const char* aFileName) {
    SoundHeader header;
    header.setHighMono();
    Construct(aFileName, header);
}
   ...
   if (sndHeader.getSamples() != sampleCount/sndHeader.getChannels()) {
      switch (sndHeader.getOutputType()) {
         case TYPE_SND:
            ...
            outFile.writeBigEndian((int32_t)
         ...
         case TYPE_WAV_PCM:
            ...
            outFile.writeLittleEndian((int32_t)

...
void Array<type>::zero(int minIndex, int maxIndex) {
   ...
   if (minIndex < 0 || maxIndex < 0 || minIndex > maxIndex ||
       maxIndex >= this->size) {
      std::cerr << "Error in zero function: min = " << minIndex
                << " max = " << maxIndex << " size = " << this->size << std::endl;
      exit(1);
   }
...
Array<type>& Array<type>::operator+=(const Array<type>& anArray) {
   if (this->size != anArray.size) {
      std::cerr << "Error: different size arrays " << this->size << " and "
                << anArray.size << std::endl;
      exit(1);
   }
